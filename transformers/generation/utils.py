...
class GenerationMixin:
    ...
    def _sample(
        ...
        **model_args,
        ...
        pad_id = generation_config._pad_token_tensor
        eos_token_id = generation_config.eos_token_id
        return_dict = generation_config.return_dict_in_generate

        # output control
        need_logits = generation_config.output_logits
        need_scores = generation_config.output_scores
        need_hidden = generation_config.output_hidden_states
        need_attn = generation_config.output_attentions
        do_sample = generation_config.do_sample

        batch_size, seq_len = input_ids.shape
        unfinished_mask = torch.ones(batch_size, dtype=torch.long, device=input_ids.device)
        model_args = self._get_initial_cache_position(seq_len, input_ids.device, model_args)

        is_prefill = generation_config.prefill_chunk_size is None
        if not is_prefill:
            model_args = self._prefill_chunking(input_ids, generation_config, **model_args)

        raw_logits, score_trace, hidden_trace, attn_trace = (), (), (), ()

        forward_fn = self.get_compiled_call(generation_config.compile_config) if self._valid_auto_compile_criteria(model_args, generation_config) else self.__call__

        while self._has_unfinished_sequences(False, synced_gpus, input_ids.device):
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_args)
            if need_attn: model_inputs.update(output_attentions=True)
            if need_hidden: model_inputs.update(output_hidden_states=True)

            mid_layers = list(range(20, 30))
            if is_prefill:
                spectral_outputs, output = self(**model_inputs, return_dict=True, mid=mid_layers)
                is_prefill = False
            else:
                spectral_outputs, output = forward_fn(**model_inputs, return_dict=True, mid=mid_layers)

            model_args = self._update_model_kwargs_for_generation(output, model_args, self.config.is_encoder_decoder)

            last_logits = output.logits[:, -1, :]
            base_probs = F.softmax(last_logits, dim=-1)

            topk_probs, topk_ids = torch.topk(base_probs, k=threshold_top_k, dim=-1)
            topk_cumsum = topk_probs.cumsum(dim=-1)

            fused_logits_list = []

            for i in range(last_logits.shape[0]):
                cutoff_idx = torch.searchsorted(topk_cumsum[i], threshold_top_p, right=False)
                cutoff = min(cutoff_idx + 1, threshold_top_k)
                candidate_ids = topk_ids[i, :cutoff]

                try:
                    stacked_logits = torch.stack([spectral_outputs[l][i, -1, :] for l in mid_layers], dim=0)
                except KeyError as e:
                    raise ValueError(f"Missing logits for layer {e}")

                layer_probs = F.softmax(stacked_logits, dim=-1)
                selected_probs = layer_probs[:, candidate_ids]
                max_idx = torch.argmax(selected_probs)
                best_layer_idx = max_idx // selected_probs.shape[1]

                chosen_layer = mid_layers[best_layer_idx.item()]
                chosen_logits = spectral_outputs[chosen_layer][i, -1, :]

                mask = torch.ones_like(chosen_logits, dtype=torch.bool)
                mask[candidate_ids] = False
                chosen_logits = chosen_logits.masked_fill(mask, -float("inf"))

                max_prob = selected_probs.max()
                final_logits = last_logits[i] + alpha * max_prob * chosen_logits
                fused_logits_list.append(final_logits)

            logits_fused = torch.stack(fused_logits_list, dim=0)
            token_scores = logits_processor(input_ids, logits_fused)
            ...
            if do_sample:
                ...
                next_tokens = torch.argmax(token_scores, dim=-1)

            if eos_token_id is not None:
                next_tokens = next_tokens * unfinished_mask + pad_id * (1 - unfinished_mask)

            input_ids = torch.cat([input_ids, next_tokens[:, None]], dim=-1)
            if streamer:
                streamer.put(next_tokens.cpu())

            unfinished_mask = unfinished_mask & ~stopping_criteria(input_ids, score_trace)
            if unfinished_mask.max() == 0:
                break

            if return_dict:
                if need_scores:
                    score_trace += (token_scores,)
                if need_logits:
                    raw_logits += (logits_fused,)
                if need_attn:
                    trace = output.decoder_attentions if self.config.is_encoder_decoder else output.attentions
                    attn_trace += (trace,)
                if need_hidden:
                    hidden = output.decoder_hidden_states if self.config.is_encoder_decoder else output.hidden_states
                    hidden_trace += (hidden,)
        ...
        if return_dict:
            output_cls = GenerateEncoderDecoderOutput if self.config.is_encoder_decoder else GenerateDecoderOnlyOutput
            return output_cls(
                sequences=input_ids,
                scores=score_trace,
                logits=raw_logits,
                decoder_attentions=attn_trace,
                decoder_hidden_states=hidden_trace,
                past_key_values=model_args.get("past_key_values"),
            )
        return input_ids
    ...
    def _beam_search_custom(
        ...
        **model_args,
        ...
        print('[beam_search_LISA] decoding start')

        # --- Parameter Setup ---
        pad_id = generation_config._pad_token_tensor
        eos_id = generation_config._eos_token_tensor
        num_beams = generation_config.num_beams
        num_return_sequences = generation_config.num_return_sequences
        max_len = generation_config.max_length
        do_sample = generation_config.do_sample
        length_penalty = generation_config.length_penalty
        early_stop = generation_config.early_stopping

        return_dict = generation_config.return_dict_in_generate
        out_logits = generation_config.output_logits
        out_scores = generation_config.output_scores
        out_attn = generation_config.output_attentions
        out_hidden = generation_config.output_hidden_states

        bsz_all, cur_len = input_ids.shape
        bsz = bsz_all // num_beams
        vocab_size = self.config.get_text_config().vocab_size

        eos_mult = eos_id.shape[0] if eos_id is not None else 0
        max_beams = max(2, 1 + eos_mult) * num_beams
        beam_mask = torch.cat([
            torch.ones(num_beams, dtype=torch.bool),
            torch.zeros(max_beams - num_beams, dtype=torch.bool)
        ]).to(input_ids.device)

        model_args = self._get_initial_cache_position(cur_len, input_ids.device, model_args)

        # --- Output Containers ---
        score_history = () if return_dict and out_scores else None
        logits_history = () if return_dict and out_logits else None
        beam_trace = () if return_dict and out_logits else None
        attn_history = () if return_dict and out_attn else None
        cross_attn_history = () if return_dict and out_attn else None
        hidden_history = () if return_dict and out_hidden else None

        if return_dict and self.config.is_encoder_decoder:
            encoder_attn = model_args["encoder_outputs"].get("attentions") if out_attn else None
            encoder_hidden = model_args["encoder_outputs"].get("hidden_states") if out_hidden else None

        # --- Beam States Initialization ---
        filler = pad_id or eos_id[0] if eos_id is not None else -1
        beam_buffer = torch.full((bsz, num_beams, max_len), filler, dtype=torch.long, device=input_ids.device)
        beam_buffer[:, :, :cur_len] = self._unflatten_beam_dim(input_ids, bsz, num_beams)
        final_seq = beam_buffer.clone()

        path_scores = torch.zeros((bsz, num_beams), dtype=torch.float, device=input_ids.device)
        path_scores[:, 1:] = -1e9
        best_scores = torch.full_like(path_scores, -1e9)
        beam_done = torch.zeros_like(path_scores, dtype=torch.bool)
        stop_flags = torch.zeros_like(path_scores, dtype=torch.bool)

        beam_positions = torch.full((bsz, num_beams, max_len - cur_len), -1, dtype=torch.int, device=input_ids.device)
        beam_trace = beam_positions.clone()

        # --- Generation Loop ---
        while self._has_unfinished_sequences(False, synced_gpus, device=input_ids.device):
            flat_beams = self._flatten_beam_dim(beam_buffer[:, :, :cur_len])
            inputs = self.prepare_inputs_for_generation(flat_beams, **model_args)
            inputs.update({"output_attentions": out_attn} if out_attn else {})
            inputs.update({"output_hidden_states": out_hidden} if out_hidden else {})

            mid_layers = list(range(20, 30))
            new_logits, output = self(**inputs, return_dict=True, mid=mid_layers)
            model_args = self._update_model_kwargs_for_generation(output, model_args, is_encoder_decoder=self.config.is_encoder_decoder)

            if synced_gpus and False:
                continue

            last_logits = output.logits[:, -1, :]
            last_probs = F.softmax(last_logits, dim=-1)
            top_probs, top_ids = torch.topk(last_probs, k=threshold_top_k, dim=-1)
            top_cumsum = top_probs.cumsum(dim=-1)

            fused_logits_all = []

            for i in range(last_logits.shape[0]):
                cutoff_idx = torch.searchsorted(top_cumsum[i], threshold_top_p, right=False)
                cutoff = min(cutoff_idx + 1, threshold_top_k)
                cand_ids = top_ids[i, :cutoff]

                try:
                    stack_logits = torch.stack([new_logits[layer][i, -1, :] for layer in mid_layers], dim=0)
                except KeyError as e:
                    print(f"[Error] Missing layer: {e}")
                    raise

                stack_probs = F.softmax(stack_logits, dim=-1)
                cand_probs = stack_probs[:, cand_ids]
                max_idx = torch.argmax(cand_probs)
                sel_layer = mid_layers[max_idx // cand_probs.shape[1]]
                token_logits = new_logits[sel_layer][i, -1, :]

                mask = torch.ones_like(token_logits, dtype=torch.bool)
                mask[cand_ids] = False
                token_logits = token_logits.masked_fill(mask, -float("inf"))

                merged_logits = last_logits[i] + alpha * token_logits
                fused_logits_all.append(merged_logits)

            logits = torch.stack(fused_logits_all, dim=0)
            log_probs = F.log_softmax(logits, dim=-1)
            log_probs = logits_processor(flat_beams, log_probs)
            ...
            if return_dict:
                ...
                if out_scores:
                    score_history += (log_probs.clone(),)
                if out_attn:
                    attn_history += ((output.decoder_attentions,) if self.config.is_encoder_decoder else (output.attentions,))
                    if self.config.is_encoder_decoder:
                        cross_attn_history += (output.cross_attentions,)
                if out_hidden:
                    hidden_history += ((output.decoder_hidden_states,) if self.config.is_encoder_decoder else (output.hidden_states,))

            # --- Beam Score Update ---
            log_probs = self._unflatten_beam_dim(log_probs, bsz, num_beams)
            log_probs += path_scores[:, :, None]
            flat_scores = log_probs.reshape(bsz, num_beams * vocab_size)

            topk_scores, topk_seq, topk_idx = self._get_top_k_continuations(
                accumulated_log_probs=flat_scores,
                running_sequences=beam_buffer,
                running_beam_indices=beam_positions,
                cur_len=cur_len,
                decoder_prompt_len=cur_len,
                do_sample=do_sample,
                beams_to_keep=max_beams,
                num_beams=num_beams,
                vocab_size=vocab_size,
                batch_size=bsz,
            )

            stop_flags = stopping_criteria(self._flatten_beam_dim(topk_seq[:, :, :cur_len+1]), score_history)
            stop_flags = self._unflatten_beam_dim(stop_flags, bsz, max_beams)

            beam_buffer, path_scores, beam_positions = self._get_running_beams_for_next_iteration(
                topk_log_probs=topk_scores,
                topk_running_sequences=topk_seq,
                topk_running_beam_indices=topk_idx,
                next_token_hits_stopping_criteria=stop_flags,
                num_beams=num_beams,
            )

            final_seq, best_scores, beam_trace, beam_done = self._update_finished_beams(
                sequences=final_seq,
                topk_running_sequences=topk_seq,
                beam_scores=best_scores,
                topk_log_probs=topk_scores,
                beam_indices=beam_trace,
                topk_running_beam_indices=topk_idx,
                is_sent_finished=beam_done,
                next_token_hits_stopping_criteria=stop_flags,
                top_num_beam_mask=beam_mask,
                num_beams=num_beams,
                cur_len=cur_len,
                decoder_prompt_len=cur_len,
            ...
            if model_args.get("past_key_values", None) is not None:
                model_args["past_key_values"] = self._temporary_reorder_cache(
                    past_key_values=model_args["past_key_values"],
                    beam_idx=self._flatten_beam_dim(beam_positions[..., cur_len - cur_len])
                )

            cur_len += 1
            this_done = not self._beam_search_has_unfinished_sequences(
                path_scores, best_scores, beam_done, stop_flags,
                cur_len, max_len, cur_len, early_stop, length_penalty
            )

        # --- Final Output ---
        final_seq = self._flatten_beam_dim(final_seq[:, :num_return_sequences, :])
        best_scores = self._flatten_beam_dim(best_scores[:, :num_return_sequences])
        beam_trace = self._flatten_beam_dim(beam_trace[:, :num_return_sequences, :])

        gen_len = ((beam_trace + 1).bool()).sum(dim=1).max()
        output_len = cur_len + gen_len
        final_seq = final_seq[:, :output_len]
        beam_trace = beam_trace[:, :gen_len]

        if return_dict:
            result_cls = GenerateBeamEncoderDecoderOutput if self.config.is_encoder_decoder else GenerateBeamDecoderOnlyOutput
            return result_cls(
                sequences=final_seq,
                sequences_scores=best_scores if out_scores else None,
                scores=score_history,
                logits=logits_history,
                beam_indices=beam_trace,
                encoder_attentions=encoder_attn if self.config.is_encoder_decoder else None,
                encoder_hidden_states=encoder_hidden if self.config.is_encoder_decoder else None,
                decoder_attentions=attn_history,
                cross_attentions=cross_attn_history,
                decoder_hidden_states=hidden_history,
                past_key_values=model_args.get("past_key_values"),
            )
        else:
            return final_seq
