...
class Qwen2_5_VLSdpaAttention(Qwen2_5_VLAttention):
    """
    Qwen2.5 Attention with Layer-wise Spectral Modulation.
    
    This module implements spectral scaling over query and key projections 
    based on transformer depth, to stabilize attention and mitigate hallucinations.
    
    Inspired by functional zoning:
    - Preservation (shallow layers): retains input signals
    - Interaction (mid layers): fuses semantic features
    - Suppression (deep layers): suppresses unstable attention spikes
    """
    ...
    def forward(
        ...
        if output_attentions:
            logger.warning_once(
                "Scaled Dot Product Attention doesn't support `output_attentions=True` with SDPA. "
                "Falling back to manual attention implementation."
            )
        ...
        # === Repeat for multi-group attention heads ===
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
        ...
        # === Spectral modulation ===
        alpha, beta, gamma = self._get_layerwise_modulation_params(self.layer_idx)

        norm_q = 1 + gamma / torch.log(trace_W_Q_2_query_states + 1e-6)
        norm_k = 1 + gamma / torch.log(trace_W_Q_2_key_states + 1e-6)

        query_scaled = query_states * norm_q * alpha
        key_scaled = key_states * norm_k * beta
    ...
    def _get_layerwise_modulation_params(self, layer_idx: int) -> Tuple[float, float, float]:
        """
        Returns spectral modulation coefficients (alpha, beta, gamma)
        for the given transformer layer index.
        """
        if layer_idx < 18:
            return 0.9, 0.9, 0.3  # Preservation zone
        elif 18 <= layer_idx <= 26:
            return 1.0, 1.1, 0.6  # Interaction zone
        else:
            return 1.2, 1.3, 1.0  # Suppression zone
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        interaction_zones: Optional[List[int]] = None,  # add
        ...
        # )
        pool = {} if interaction_zones is not None else None
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if interaction_zones is not None and idx in interaction_zones:
                pool[idx] = hidden_states.clone()
        ...
        if interaction_zones is not None:
            return pool, BaseModelOutputWithPast(
                last_hidden_state=hidden_states,
                past_key_values=next_cache,
                hidden_states=all_hidden_states,
                attentions=all_self_attns,
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        #     cache_position=cache_position,
        # )
        pool, outputs = self.language_model(
    input_ids=None,
    position_ids=position_ids,
    attention_mask=attention_mask,
    past_key_values=past_key_values,
    ...
    return_dict=True,
    cache_position=cache_position,
    interaction_zones=interaction_zones,  # add
)
        ...
        return (pool, output) if return_dict else output.to_tuple()
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if pool is not None:
            for k in pool:
                pool[k] = self.lm_head(pool[k])

        return (pool, Qwen2_5_VLCausalLMOutputWithPast(
    loss=loss,
    logits=logits,
    past_key_values=outputs.past_key_values,
    hidden_states=outputs.hidden_states,
